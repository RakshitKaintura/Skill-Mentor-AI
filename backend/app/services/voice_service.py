...
class VoiceService:
    ...
    def get_instruction(self, topic: str, skill: str, level: str) -> str:
    ...
    async def handle_voice_lifecycle(self, websocket: WebSocket, instruction: str, topic: str, skill: str) -> None:
        ...
        try:
            initial_resp = await self._generate_content_with_retry(greeting_prompt, instruction)
        ...
        while True:
            ...
            if msg_type == "text":
                ...
                await self._stream_voice_response(websocket, instruction, user_input)
    ...
    async def _stream_voice_response(self, websocket: WebSocket, instruction: str, user_input: str) -> None:
        """Streams the AI response sentence-by-sentence to optimize for Text-to-Speech frontends."""
        try:
            response = await self._generate_content_with_retry(user_input, instruction)
    ...
    async def _generate_content_with_retry(self, prompt: str, instruction: str, attempts: int = 3) -> Any:
